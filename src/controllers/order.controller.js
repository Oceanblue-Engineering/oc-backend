...
// Update entire order (POS-Style Edit with smart stock delta deduction/restoration)
export const updateEntireOrder = asyncErrorHandler(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new CustomError(400, "Order must contain at least one product")
    );
  }
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      if (order.isDeleted) {
        throw new CustomError(400, "Cannot update a deleted order");
      }

      const activeStorefrontId = storefrontId || order.storefrontId;

      // 2. Validate and normalize Credit Person (Optional for Paid, Required for Credit)
      let normalizedCreditPersonId = null;
      if (creditPersonId) {
        const rawCpId =
          typeof creditPersonId === "object"
            ? creditPersonId._id
            : creditPersonId;
        if (
          rawCpId &&
          typeof rawCpId === "string" &&
          rawCpId.trim() !== "" &&
          mongoose.Types.ObjectId.isValid(rawCpId)
        ) {
          normalizedCreditPersonId = new mongoose.Types.ObjectId(rawCpId);
          const creditPerson = await CreditPerson.findById(
            normalizedCreditPersonId
          ).session(session);
          if (!creditPerson) {
            throw new CustomError(404, "Credit person not found");
          }
          if (creditPerson.blacklist) {
            throw new CustomError(
              400,
              `Cannot link blacklisted customer to order: ${
                creditPerson.blacklistReason || "Blacklisted"
              }`
            );
          }
        }
      ...
      if (inventoryItems.length !== incomingInventoryIds.length) {
        throw new CustomError(404, "One or more inventory items were not found");
      }
      ...
      // 4. Map old order products and new order products
      ...
      // 5. Smart Stock Delta Calculation (Multi-batch FIFO deduction & restoration)
      ...
      for (const idStr of allProductIds) {
        ...
        if (delta > 0) {
          // Increase quantity -> Deduct from storefront batches (FIFO)
          const stockRecords = await StorefrontInventory.find(
            {
              inventoryId: new mongoose.Types.ObjectId(idStr),
              storefrontId: new mongoose.Types.ObjectId(activeStorefrontId),
              quantity: { $gt: 0 },
            },
            null,
          ...
          if (totalAvailable < delta) {
            ...
            throw new CustomError(
              ...
              `Insufficient stock for '${
                inv?.productName || idStr
              }'. Available: ${totalAvailable}, Additional requested: ${delta}`
            );
          }
          ...
          // Decrease quantity -> Restore stock to storefront batch
          ...
          if (!stockRecord) {
            ...
            stockRecord.quantity += restoreQty;
            stockRecord.lastUpdated = new Date();
            await stockRecord.save({ session });
          }
        }
      ...
      // 6. Build new ordersProducts array with snapshot prices
      ...
      const finalSubTotal =
        subTotal !== undefined && subTotal !== null
          ...
          : calculatedSubTotal;
      const numTax =
        tax !== undefined && tax !== null ? Number(tax) : (order.tax || 0);
      const numDiscount =
        discount !== undefined && discount !== null
          ? Number(discount)
          : (order.discount || 0);

      // Handle delivery fee if present
      let deliveryFee = 0;
      if (deliveryDetails && deliveryDetails.township) {
        if (!mongoose.Types.ObjectId.isValid(deliveryDetails.township)) {
          throw new CustomError(400, "Invalid township ID format");
        ...
        if (township && township.isActive) {
          deliveryFee = township.deliveryFee || 0;
      ...
      } else if (order.deliveryDetails?.deliveryFee) {
      ...
      const calculatedFinalAmount =
        finalAmount !== undefined && finalAmount !== null
          ? Number(finalAmount)
          : Math.max(0, finalSubTotal + deliveryFee + numTax - numDiscount);
      const numPaidAmount =
      ...
      if (creditPersonId !== undefined) {
        order.creditPersonId = normalizedCreditPersonId;
      }
    ...
    if (error.name === "ValidationError") {
      ...
      return next(
        new CustomError(400, `Validation error: ${messages.join(". ")}`)
      );
    }
    return next(
